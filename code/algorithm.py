...
def propagation_and_random_search(
    ...
    target_patches[nans] = 510
    nans = np.isnan(source_patches)
    source_patches[nans] = -255
    ...
    if random_enabled:
        new_f1,best_D1 = random_search(source_patches,target_patches,new_f,alpha,w,best_D,odd_iteration)
    if propagation_enabled:
        new_f2,best_D2 = propagation_search(source_patches,target_patches,new_f1,best_D1,odd_iteration)



    #############################################

    return new_f2, best_D2, global_vars


#helper funtion to perform random_search
def random_search(source_patches, target_patches, f, alpha, w, best_D,odd_iteration):
    new_f = f.copy()
    size = 1
    ...
    for i in x_axis:
        for j in y_axis:
                ...
                for k in range(size):
                    x2 = x1 + w*(alpha**(k+1))*x_offset[k]
                    y2 = y1 + w*(alpha**(k+1))*y_offset[k]
                    x2 = int(x2)
                    y2 = int(y2)
                    x2 = max(x2,0)
                    y2 = max(y2,0)
                    x2 = min(x2, x_size-1)
                    y2 = min(y2, y_size-1)
                    new_f1, best_D1 = smallerdistance(source_patches,target_patches,best_D,i,j,x2,y2,new_f)#this temp_x and temp_y has smaller distance
    return new_f1,best_D1
...
def propagation_search(source_patches,target_patches,f,best_D,odd_iteration):
    ...
    for i in x_axis:
        for j in y_axis:
            ...
            x3 = min(x3,x_size-1)
            y3 = min(y3,y_size-1)
            new_f1, best_D1 = smallerdistance(source_patches,target_patches,best_D,i,j,x1,y1,new_f)
            new_f2, best_D2 = smallerdistance(source_patches,target_patches,best_D1,i,j,x3,y1,new_f1)
            new_f3, best_D3 = smallerdistance(source_patches,target_patches,best_D2,i,j,x1,y3,new_f2)
    return new_f3,best_D3
...
def smallerdistance(source_patches,target_patches,best_D,x1,y1,x2,y2,f):
    ...
    if ans<best_D[x1][y1] or best_D[x1][y1]==0:
        best_D[x1][y1] = ans
        f[x1][y1] = (x2-x1,y2-y1)
